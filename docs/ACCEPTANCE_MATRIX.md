...
> 状态：`SPEC_FROZEN / P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS / P6_TARGET_PASS / P7_CONTENT_PASS / P8_LAYOUT_PASS / P9_AUTHORIZED_IN_PROGRESS`
>
...
P9 不改变第 2 节冻结的驱动语义，只复跑最终版本并检查交付闭环。完整命令、预期
结果和原始输出清单见 `docs/P9_FINAL_RUNBOOK.md`。

| ID | 验收项 | 通过条件 | 必需原始证据 |
|---|---|---|---|
| T-P9-01 | 明确提交与干净构建 | 目标 Linux 的 `HEAD` 与远端明确提交一致；driver/tools 严格构建退出码 0 | 环境与构建完整日志、提交哈希 |
| T-P9-02 | P1–P5 最终回归 | 500 次并发、20 轮生命周期、FD/业务/poll/bridge 全部退出码 0 | `vled_verify.sh` 未编辑完整输出、退出码 |
| T-P9-03 | 内核稳定性 | 本轮无 warning/oops/BUG/lockdep/use-after-free/general protection fault，模块和节点干净卸载 | 本轮完整 `journalctl -k` 区间 |
| T-P9-04 | 故障恢复 | 已加载模块和错误设备路径被明确拒绝；按 README 可恢复 | 两个负面测试原始输出与退出码 |
| T-P9-05 | 非开发成员盲演 | 只看根 README 完成 Linux→Windows 演示，无开发者过程提示 | Linux demo 日志、Windows GUI 日志/PNG、操作者真实说明 |
| T-P9-06 | 交付完整性 | P8 DOCX/PDF 可打开，PDF 51 页 A4；证据哈希通过；无构建产物、Word 锁文件、临时文件或敏感凭据进入提交 | 文件哈希、结构检查、仓库审计输出 |

T-P9-01..06 在目标 Linux 与人工盲演原始材料全部复核前保持 `PENDING`；不得以
P1–P8 的历史 PASS 替代 P9 最终运行。
