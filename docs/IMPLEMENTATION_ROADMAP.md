...
| 本地基线提交 | `a192f2c`（与本地 `integration` 相同） |
| 远端状态 | 本地 `integration-promote-try` 落后 `origin/integration-promote-try` 1 个提交 |
| 当前执行状态 | `P0_COMPLETE_WAITING_FOR_P1_P2_APPROVAL` |
| 当前允许动作 | 只读检查、说明 P1/P2 具体批次并等待许可 |
| P0 产物 | `ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 用户批准 P1、P2 或指定其一后实施第一批代码与对应测试 |
...
| P1 | 缓冲区与独立写偏移修正 | P0 | 未开始 |
| P2 | 模拟器线程安全与协议校验 | P0 | 未开始 |
...
- [ ] P1 缓冲区与独立写偏移修正。
- [ ] P2 模拟器线程安全与协议校验。
- [ ] 任何后续工具、README、PLAN 或报告正文工作。

### 当前阻塞/门禁

```text
P0_COMPLETE_WAITING_FOR_P1_P2_APPROVAL
```

### 下一动作

1. 向用户汇报 P1/P2 的首批具体文件、测试和暂停点。
2. 用户可批准 P1、P2 或两者；未获批部分不修改。
3. 首个 Linux 运行批次形成“实现 + 对应测试 + 运行说明”的提交后，再进行一次受 lease 保护的远端对齐。
4. Linux 端先卸载当前旧模块、保护/清理测试产物，再覆盖到明确提交哈希。
