...
>
> 当前执行门禁：**P1–P5 已关闭，用户已授权 P6。P6 完成前不得进入 P7，也不得修改课程设计报告。**
...
| 当前执行状态 | `P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS` |
| 当前允许动作 | 执行 P6：统一 README、PLAN、命令名称和演示资料；不得进入 P7 |
| P0 产物 | `IMPLEMENTATION_ROADMAP.md`、`ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 完成 P6 文档审计、统一、静态检查和 Windows 回归；Linux 只验证已普通 fast-forward 推送的明确提交 |
...
| P6 | README、PLAN 与命令统一 | P3；P5 后复核 | 进行中（用户已授权） |
...
1. 提交并普通 fast-forward 推送 P6 文档与统一验收入口的明确提交。
2. 在目标 Linux 只验证该已推送提交的 README 命令和 `vled_verify.sh`。
3. 固化 P6 验证结论并关闭 P6；关闭前不得进入 P7 或修改课程设计报告。
