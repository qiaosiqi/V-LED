...
>
> 当前执行门禁：**P0 已完成。未经新的阶段许可，不得开始 P1/P2 的驱动、模拟器或测试代码修改。**
...
| 当前实现检查点 | `47dadca`（P0、P1、P2 Windows 批次均已本地提交） |
| 远端状态 | 本地 `integration-promote-try` 相对旧远端 ahead 6、behind 1；尚未推送 |
| 当前执行状态 | `P1_IMPLEMENTED_NOT_RUN / P2_WINDOWS_AUTOMATED_PASS_GUI_REVIEW_PENDING` |
| 当前允许动作 | 汇报 Windows 检查；展示首次 `--force-with-lease` 准确命令并等待许可 |
| P0 产物 | `IMPLEMENTATION_ROADMAP.md`、`ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 用户确认首次 lease 保护推送后对齐远端，再在目标 Linux 构建并运行 P1 探针 |
...
| P1 | 缓冲区与独立写偏移修正 | P0 | 已实现并提交；目标 Linux 未运行 |
| P2 | 模拟器线程安全与协议校验 | P0 | Windows 自动测试通过；人工 GUI 复核待完成 |
| P3 | 全功能自动验收与演示脚本 | P1、P2 | 未开始 |
| P4 | 目标 Linux 实测与证据固化 | P3、目标 Linux | 未开始 |
...
- [ ] 将网络接收与 Tk 更新通过 `queue.Queue` 隔离。
- [ ] 将 JSON 解析和校验抽成不依赖 GUI 的纯函数/模块。
- [ ] 使用完整状态对象替换，消除多字段竞态。
- [ ] 为 socket 设置可退出机制并实现窗口关闭清理。
- [ ] 限制日志条数，避免长时间演示造成内存无界增长。
- [ ] 验证手动颜色、亮度覆盖和恢复 UDP 值的业务逻辑。
- [ ] 区分“本地清屏”和“驱动 CLEAR”，避免界面文案误导。
- [ ] 记录无效报文原因，但禁止部分更新 GUI。

### 自动测试

- [ ] 合法 static/scroll 状态。
- [ ] 缺字段、类型错误、范围错误、非法 mode。
- [ ] 非法 UTF-8、非 JSON、非 state 报文。
- [ ] 连续高频报文只在主线程更新 UI。
- [ ] 手动覆盖期间 UDP 更新状态但不覆盖显示值；恢复后立即采用最新 UDP 值。
- [ ] 关闭窗口后监听线程和 socket 能退出。
...
- [ ] `make` 成功并记录编译器、内核和模块元信息。
- [ ] `insmod` 成功，`dmesg` 出现注册信息。
- [ ] 自动分配 major/minor。
- [ ] `/dev/vled` 自动出现，类型和权限正确。
- [ ] `rmmod` 成功，设备节点消失且资源释放。
- [ ] 重复加载/卸载循环无泄漏、崩溃或残留节点。

#### B. 文件操作与命令业务

- [ ] `open/read/write/release` 可由用户态工具显式触发。
- [ ] `TEXT`：空文本、ASCII、中文、引号、反斜杠。
- [ ] `COLOR`：0、255、越界、缺参数、多参数、非数字。
- [ ] `BRIGHTNESS`：0、100、越界和非数字。
- [ ] `MODE`：static、scroll 和非法值。
- [ ] `CLEAR`：清空文字但保留约定的其他状态。
- [ ] `STATUS`：不改变状态版本。
- [ ] 未知命令和暂不支持命令返回正确错误码。
- [ ] 每次有效状态变化的 version 精确递增一次。

#### C. 缓冲区、偏移与并发

- [ ] 零长度、PAGE_SIZE-1、PAGE_SIZE、超 PAGE_SIZE。
- [ ] 两个独立 FD 从各自偏移 0 开始读取。
- [ ] 分段读期间另一 FD 更新状态，旧 FD 仍得到完整旧快照。
- [ ] 新 FD 得到最新状态。
- [ ] 两个 FD 的写容量和写偏移互不影响。
- [ ] 并发写不会产生字段混合或无效 JSON。
- [ ] 并发读写压力后设备仍可用且模块可卸载。
...
- [ ] CLI 参数、设备路径、退出码和错误信息。
- [ ] bridge IP、端口、间隔和信号退出。
- [ ] bridge 只发送通过校验的完整状态。
- [ ] Windows 收到文字、颜色、亮度、模式和版本变化。
- [ ] 网络不可达、端口占用和模拟器未启动时有可诊断日志。
...
| 2026-07-12 | P2 GUI 可启动，但自动窗口捕获返回错误应用画面 | 不对不一致句柄发送输入，不把人工 GUI 门禁记为通过 | `GUI_REVIEW_PENDING` |
...
- [ ] P1 尚未在目标 Ubuntu 6.17.0-35-generic 构建、加载和运行探针，因此不得标记 PASS。
...
- [ ] P2 人工 GUI 视觉/交互复核因窗口捕获句柄错配未完成；跨机 UDP 留到 P3/P4。
- [ ] P3 及后续阶段均未获本轮实施许可，不得开始。

### 当前阻塞/门禁

```text
P1_IMPLEMENTED_NOT_RUN
P2_WINDOWS_AUTOMATED_PASS_GUI_REVIEW_PENDING
WAITING_FOR_FORCE_WITH_LEASE_APPROVAL_BEFORE_TARGET_LINUX
```

### 下一动作

1. 向用户汇报 P0/P1/P2 提交、Windows 自动检查、mutation 结果和 GUI 限制。
2. 展示本地/远端提交图、明确 lease 对象和准确的首次 `git push --force-with-lease` 命令；等待用户动作时确认。
3. 获得确认后才对齐远端；在此之前不执行任何 push。
4. Linux 端先卸载当前旧模块、保护/清理测试产物，再覆盖到明确提交哈希并运行 P1 构建与探针。
