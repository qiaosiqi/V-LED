...
>
> 当前执行门禁：**P1–P8 已关闭。用户已授权以 `a6c517c` 为起点启动 P9；P9 在目标 Linux 最终回归、非开发成员盲演和原始证据均回传通过前不得关闭。**
...
| 当前已验证实现 | `cf2a6c2`（P1–P4 验收与正式证据均已固化） |
| P5 已验证实现 | `e142f20`（T-POLL-01..08、P1–P4 回归和目标 Linux 内核日志均通过） |
| 同步规则状态 | 首次 `--force-with-lease` 已完成；后续只允许 fast-forward 推送 |
| 当前执行状态 | `P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS / P6_TARGET_PASS / P7_CONTENT_PASS / P8_LAYOUT_PASS / P9_AUTHORIZED_IN_PROGRESS` |
| 当前允许动作 | P9 本机预检、最终目标 Linux 回归、Windows 非开发成员盲演、故障恢复彩排和交付检查 |
| P0 产物 | `IMPLEMENTATION_ROADMAP.md`、`ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 固化 P9 运行手册并普通 fast-forward 推送；在目标 Linux/Windows 对明确提交执行最终回归和盲演，回传未编辑原始输出 |

### 0.1 当前工作区保护事项

1. `docs/操作系统课程设计报告2026.doc` 存在用户未提交修改。
2. 任何 Git 操作都不得覆盖、还原、暂存或顺带提交该文件，除非用户单独授权。
3. 旧验证提交 `a40be52` 已被舍弃，只能作为设计参考，不得合并或 cherry-pick。
4. 首次历史对齐已经完成；当前本地与远端跟踪分支从 `6c51e5d` 一致前进，后续只允许普通 fast-forward 推送，禁止 force push。
5. 每次开始工作前必须执行并记录：
...
7. 首次历史对齐已经完成；后续推送前复核本地与远端祖先关系，只允许普通 fast-forward 推送。
...
| P5 | `poll + wait queue` 扩展 | P4 基础测试通过 | 已通过目标 Linux T-POLL-01..08、P1–P4 回归与内核日志验收 |
| P6 | README、PLAN 与命令统一 | P3；P5 后复核 | 已通过目标 Linux 统一入口与 P1–P5 回归 |
| P7 | 约 50 页报告骨架与内容填充 | P0 可建骨架；实测内容依赖 P4/P5 | 已生成 49 页正文工作稿并完成逐页内容/版式 QA |
| P8 | 图表、引用、目录、版式与逐页 QA | P7 内容稳定 | 51 页排版稿/PDF已完成；题注、交叉引用、图表目录、无障碍与逐页 QA 通过 |
| P9 | 最终回归、演示彩排与交付检查 | P4–P8 | 已授权，执行中 |
...
| poll/wait queue 探针 | `vled_poll_probe` |
| 演示编排脚本 | `vled_demo.sh` |
...
- [x] 新增或完善仓库根 README，提供最短可运行路径。
- [x] 更新 `DriverRequirements.md`，只记录 PDF 明示要求和验收映射。
- [x] 将 `PLAN.md` 从旧草案更新为实际架构与真实文件名。
- [x] 更新 `driver/README.md`：缓冲区、偏移、错误码、poll、加载卸载。
- [x] 更新 `tools/README.md`：规范命令、自动测试、bridge、演示流程。
- [x] 更新 `simulator/README.md`：删除旧分支说明，解释线程模型和协议校验。
- [x] 所有文档统一使用 `vled_cli`、`vled_bridge`、`vled_verify.sh`、`vled_fd_probe`、`vled_poll_probe` 和 `vled_demo.sh`。
- [x] 命令示例必须从空环境开始可复制执行。
- [x] 文档注明哪些步骤需要 root、哪些只需要普通用户。
- [x] 增加常见故障：headers 缺失、权限、UDP 防火墙、IP、端口占用。
...
- [x] 统一标题编号，不再混用“0.x”“一、”“四、”和手写序号。
- [x] 自动生成目录、图目录和表目录。
- [x] 所有图表使用题注，并在正文通过交叉引用引用。
- [x] 所有代码清单注明文件、函数和用途。
- [x] 统一中文字体、英文字体、字号、行距、页边距、页眉和页码。
- [x] 表格跨页重复表头，不截断文本。
- [x] 图片保持可读分辨率，不拉伸，不用整页终端截图代替分析。
- [x] 修复 `/PATH-TO-REPO` 等占位符和旧命令。
- [x] 检查三人心得均不少于 500 字且内容独立。
- [x] 检查课程设计日志完整、真实且与 Git/测试时间线一致；无法可靠推断项按用户授权保留 `【待人工填写】`。
- [x] 渲染为逐页 PNG/PDF，逐页检查裁切、重叠、空白、编号和引用。
...
| 2026-07-12 | P5 第三次目标运行 P1 与 T-POLL-01..08 全过，完整回归 T-CON 失败 | Python 分段 reader 在第一个完整 JSON 后继续读取，竞争窗口内拼接下一版本或得到空边界；改为累计数据首次通过完整协议校验时立即返回，并增加连续双版本纯 Python 回归 | 测试适配修复中；P5 尚未关闭 |
| 2026-07-12 | `e142f20` P5 目标 Linux 最终验收通过 | P1 回归、T-CLI/T-CMD/T-CON、三轮生命周期、T-POLL-01..08、退出码和内核日志全部通过；证据目录 `docs/evidence/20260712-2127-ubuntu-6.17.0-35-generic-p5/` | `P5_TARGET_PASS` |
| 2026-07-12 | 用户授权以 `6c51e5d` 为起点启动 P6 | 统一 README、PLAN、规范工具名、权限、poll/wait queue 和故障说明；不得进入 P7 或修改课程设计报告 | 已授权 |
| 2026-07-12 | `2ea2346` P6 目标 Linux 验证通过 | 规范工具严格构建、bridge/P1/P3/P5 回归、三轮生命周期和内核日志均通过；证据目录 `docs/evidence/20260712-2147-ubuntu-6.17.0-35-generic-p6/` | `P6_TARGET_PASS` |
| 2026-07-13 | 用户授权以 `04f0b07` 为起点启动 P7，并改用 PDF 只读参考、从零制作独立正文稿 | 避免 Strict OOXML/学校模板转换消耗；原 `.doc`、旧草稿 `.docx` 与 PDF 保持不再修改 | 已授权 |
...
- [x] 用户已授权 P5；测试先行提交、wait queue/.poll/阻塞读和事件驱动 bridge 候选实现已完成。
- [x] P5 Windows 静态契约、Python 语法、16 项模拟器回归、CLI/P1/P5 探针严格语法和 shell 语法检查通过。
- [x] P5 在明确提交 `e142f20` 上通过目标 Linux 构建、T-POLL-01..08、P1–P4 回归、三轮生命周期和内核日志检查。
- [x] P5 三次失败及最终成功输出分开固化，未覆盖失败证据。
- [x] P6 根 README、PLAN、需求映射、driver/tools/simulator 文档和统一验收入口已对齐规范名称与已验证行为。
- [x] P6 在明确推送提交 `2ea2346` 上通过目标 Linux 严格构建、bridge、P1/P3/P5 回归、三轮生命周期和内核日志检查。
- [x] P7 以 `04f0b07` 为起点完成独立正文工作稿，不覆盖学校 `.doc`、旧草稿 `.docx` 或 PDF。
- [x] P7 正文稿使用真实 Heading 样式、自动目录和页码字段，Word 原生导出为 49 页，并逐页检查无裁切、重叠、断表、乱码或丢图。
- [x] P7 内容覆盖第 1–9 章、参考文献与附录，纳入 P1–P6 原始证据、两张 P4 截图、真实失败修复链和日志真实性边界。
- [x] P7 三名成员个人心得已完成人工复核；定稿后分别约 729、700、704 字，内容独立且均超过 500 字。
- [x] P8 为 22 张内容表和 9 张图建立可更新题注、书签、正文 REF 交叉引用及图表目录，并为 9 个代码清单补齐文件、函数和用途。
- [x] P8 统一标题、字体、行距、页边距、页眉页码和表格表头；9 张图均为内嵌对象并补充替代文字，无障碍审计为 0 项。
- [x] P8 清除 `/PATH-TO-REPO`、`TODO`、`待本人复核`、`本人复核稿`、`未来项` 和 `请本人` 等非日志占位；不可推断日志按用户授权使用 `【待人工填写】`。
- [x] P8 由 Microsoft Word 原生更新目录、题注、REF/PAGEREF 和页码并导出 51 页 A4 PDF；51 页 PNG 已逐页检查，无裁切、重叠、断表、乱码、丢图或断链。
...
P7_CONTENT_PASS
P8_LAYOUT_PASS
P9_AUTHORIZED_IN_PROGRESS
```

### 下一动作

1. 按 `docs/P9_FINAL_RUNBOOK.md` 固化 P9 准备提交并普通 fast-forward 推送。
2. 在目标 Linux 对该明确提交执行 20 轮生命周期、500 次并发、P1–P5 全回归和内核日志检查。
3. 由非开发成员只依据根 README 完成一次 Linux→Windows 盲演，并保留 Windows GUI 截图和两端原始输出。
4. 回传完整原始证据后逐项关闭 P9；任一失败保留本次目录，修复后使用新运行编号，不覆盖失败证据。
