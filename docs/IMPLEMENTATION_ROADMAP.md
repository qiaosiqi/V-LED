...
>
> 当前执行门禁：**P1–P8 已关闭。P9 尚未获用户授权，不得开始最终回归、盲演或交付检查。**
...
| 当前执行状态 | `P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS / P6_TARGET_PASS / P7_CONTENT_PASS / P8_LAYOUT_PASS` |
| 当前允许动作 | 固化并汇报 P8 排版稿与 51 页 PDF；等待用户决定是否批准进入 P9 |
| P0 产物 | `IMPLEMENTATION_ROADMAP.md`、`ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 提交并普通 fast-forward 推送 P8 排版稿、PDF 与阶段关闭状态；未获授权前不进入 P9 |
...
1. 提交并普通 fast-forward 推送 P8 排版稿、51 页 PDF 与阶段关闭状态。
2. 向用户汇报题注/交叉引用、图表目录、三人心得、日志占位、无障碍和 51 页逐页 QA 结论。
3. 等待用户决定是否批准进入 P9；未批准前不执行最终 Linux 回归、盲演或交付检查。
