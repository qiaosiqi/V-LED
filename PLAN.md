...
历史 P1–P6 已有目标 Linux 证据；本轮真实 fork 多进程 T-MP-01..04 已实现并接入
统一验收，但在本次构建于目标 Linux 跑完前保持 `TARGET_PENDING`。
...
- `tools/vled_multiprocess_probe.py`：真实 fork 多进程、独立写偏移、压力与共享 FD 唤醒。
...
6. `.poll` 对未消费版本返回 `POLLIN|POLLRDNORM`。成功写入唤醒等待队列重新检查；
   只有有效变化让独立 FD 可读，无变化写入只刷新共享同一 file context 的 reader。
7. 所有双锁路径固定为 `file context -> device`；进入 wait queue 前释放两把锁，
   不允许任何反向取锁路径形成循环等待。
