...
脚本会把标准输出和标准错误合并，在终端继续实时显示的同时，把每一行附加时间戳后
保存为：

```text
logs/vled-teacher-demo-YYYYmmdd-HHMMSS.log
```

`logs/` 已加入 Git 忽略规则，不会误提交运行记录。需要保存到其他目录时可设置：

```bash
VLED_LOG_DIR="$HOME/vled-demo-logs" \
    ./tools/vled_teacher_demo.sh 192.168.57.1 /dev/vled
```
