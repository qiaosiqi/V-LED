...
递增 version 并唤醒所有等待者。成功的重复设置或 `STATUS` 不增加 version，但会
让 wait queue 重新检查一次，以服务 `fork/dup` 共享同一 file context 的本地刷新；
其他 FD 的 poll 条件仍为假。失败写入不唤醒。阻塞 read 可被信号中断，非阻塞 read
在没有新版本时返回 `EAGAIN`。长期打开的 FD 因而能按版本连续获得稳定快照而不忙等。

`tools/vled_poll_probe /dev/vled` 覆盖 T-POLL-01..07，T-POLL-08 由
`tools/vled_bridge_probe.py` 覆盖；统一入口 `tools/vled_verify.sh` 同时执行
P1–P5 和新增的真实多进程回归。

## 多进程锁顺序与共享 FD 唤醒

独立进程分别 `open()` 时，共享设备状态，但拥有不同的读写偏移和快照；“偏移
独立”不等于“状态隔离”。A 进程改变状态后，B 进程的新 `open()` 或已消费旧版本
后正在等待的长期 FD 都应读到新版本。
...
`read`、`write` 和 `poll` 都遵守该顺序，不存在反向获取路径。候选状态和 JSON
在发布前准备完成，临界区内不等待用户态资源，从而缩短全局锁持有时间。

`fork()` 或 `dup()` 会让多个执行流共享同一个 open file description，也就共享同一
个 `vled_file_context`。旧实现中，读者消费当前版本后进入阻塞 read，另一个共享者
写 `STATUS` 虽设置了 `read_refresh_pending`，却因为 version 未变化而没有唤醒 wait
queue，读者可能永久等待。现在每次成功写入都会唤醒 wait queue；其他独立 FD 会
重新检查条件并继续休眠，只有共享上下文的刷新条件为真，因此不会产生虚假 poll
事件或重复 UDP 包。`tools/vled_multiprocess_probe.py` 的 T-MP-04 专门回归这一场景。
