...
历史 P1–P6 基线已有目标 Linux 证据。本轮新增共享 FD 唤醒修复、真实多进程探针
和受控死锁对照；本地语法、静态契约及 Windows 模拟器回归已通过，目标 Linux
统一验收结果应在虚拟机重跑后再回填，不沿用旧证据冒充本轮结果。
...
- `tools/vled_verify.sh`：构建、模块生命周期、P1–P5 与多进程回归入口。
- `tools/vled_fd_probe.c`：PAGE_SIZE、per-open offset、稳定快照和回滚。
- `tools/vled_poll_probe.c`：poll、wait queue、阻塞/非阻塞 read。
- `tools/vled_multiprocess_probe.py`：真实跨进程状态/偏移/压力/共享 FD 验证，以及
  有监视器兜底的用户态 ABBA 死锁与固定锁序对照演示。
...
6. `.poll` 对未消费版本返回 `POLLIN|POLLRDNORM`。只有有效变化会使独立 FD
   变为可读；成功的无变化写入仅用于刷新共享同一 file context 的 reader，不会
   生成可观察状态事件。
7. 所有需要两把锁的路径固定为“file context → device”顺序。成功但不改变版本的
   同 FD 写入允许唤醒共享该打开实例的阻塞 reader；其他 FD 的 poll 条件仍为假，
   因此不会产生状态事件或重复 UDP 发包。
