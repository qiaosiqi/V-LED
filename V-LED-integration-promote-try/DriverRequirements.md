...
| 多进程独立读写偏移 | `vled_file_context` | T-FD、T-READ、T-CON、T-MP |
...
真实进程级验收由 `tools/vled_multiprocess_probe.py` 完成。驱动所有嵌套锁路径固定
使用“per-open context 锁 → 全局设备锁”的顺序；受控 ABBA 死锁只在可终止的
用户态子进程中演示，不在内核中制造不可恢复的挂死。
