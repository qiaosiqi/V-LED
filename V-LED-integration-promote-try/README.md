...
历史基线已在 Ubuntu 24.04.4、Linux 6.17.0-35-generic、x86_64、GCC 13.3 上通过
P1–P6 验收，P7/P8 报告内容与 51 页版式已关闭；P9 教师汇报《运行指导手册》已完成。
本轮新增共享 FD 唤醒修复和 T-MP/T-DEADLOCK 测试；Windows 静态契约与模拟器回归
已通过，但修改后的 `.ko` 和真实多进程探针仍须在目标 Ubuntu 上重新运行统一验收。
...
完整自动验收会自行构建、加载、设置临时权限，执行 P1–P5、真实多进程和
受控死锁回归，然后卸载模块：
...
| `vled_multiprocess_probe.py` | 跨进程可见性、独立偏移、4+4 压力、共享 FD 唤醒和受控死锁演示 |
...
- `.poll` 只在当前 FD 有未消费版本时报告 `POLLIN|POLLRDNORM`；重复设置、
  `STATUS` 和失败命令不发布事件。
- 所有双锁路径固定使用“per-open context 锁 → 全局设备锁”的顺序，禁止反向
  获取。`fork/dup` 共享同一打开实例时，成功的 `STATUS` 会唤醒该实例的阻塞
  读者，但不会给其他 FD 制造 poll 可读事件。
