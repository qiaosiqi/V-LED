...
5. `vled_multiprocess_probe.py`：真实 fork 多进程验收，检查跨进程状态可见性、
   per-open 偏移、稳定快照、4 writer + 4 reader 压力和共享 FD 的 STATUS 唤醒；
   还提供受控用户态 ABBA 死锁与固定锁序修复的对照输出。
6. `vled_verify.sh`：P1–P5 加多进程统一验收入口，负责严格构建、bridge 黑盒测试、模块生命周期、业务/错误码/并发/poll/多进程回归和内核日志检查。
...
的模块。它会运行 `vled_fd_probe`、业务/线程并发验收、`vled_poll_probe` 和
`vled_multiprocess_probe.py`。加载、
...
# 真实驱动多进程测试，不包含故意死锁内核的危险操作。
python3 tools/vled_multiprocess_probe.py --device /dev/vled --mode driver

# 只演示可控 ABBA 死锁及固定 A→B 锁序的解决办法，不访问驱动。
python3 tools/vled_multiprocess_probe.py --mode deadlock
```
