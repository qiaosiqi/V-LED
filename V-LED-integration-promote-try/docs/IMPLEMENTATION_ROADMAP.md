...
>
> 当前执行门禁：**P1–P8 已关闭。用户于 2026-07-13 将 P9 唯一落地成果调整为可向老师汇报全部业务逻辑和功能项、且每个关键命令均有中文注释的《运行指导手册》；该手册已完成。**
...
| 当前执行状态 | `P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS / P6_TARGET_PASS / P7_CONTENT_PASS / P8_LAYOUT_PASS / P9_GUIDE_PASS` |
| 当前允许动作 | 交付并使用 `docs/运行指导手册.md` 向老师汇报；P9 不再要求补跑最终回归、盲演或证据采集 |
| P0 产物 | `IMPLEMENTATION_ROADMAP.md`、`ACCEPTANCE_MATRIX.md`、`LINUX_ENVIRONMENT.md` |
| 下一动作 | 提交并普通 fast-forward 推送《运行指导手册》和 P9 要求变更状态 |
...
| P9 | 教师汇报《运行指导手册》 | P1–P8 已验证成果 | 已完成 |
...
- [ ] 成功改变状态后调用 `wake_up_interruptible()`，使独立 FD 的版本条件成立。
- [ ] `STATUS`/重复值不得产生 poll 可读事件；允许一次内部唤醒以服务共享同一
  file context 的刷新。失败写入不得唤醒。
...
- [x] 新建 `docs/运行指导手册.md`，供成员直接向老师汇报和现场操作。
- [x] 完整覆盖系统架构、模块生命周期、PAGE_SIZE、per-open 偏移、稳定快照、
  原子回滚、version、poll/wait queue、bridge、UDP 协议和模拟器线程模型。
- [x] 完整覆盖 `TEXT/COLOR/BRIGHTNESS/MODE/CLEAR/STATUS` 六条业务命令、边界、
  错误路径、JSON 字段和 Windows 界面功能。
- [x] 给出 Windows 启动、Linux 构建/加载、业务演示、跨机演示、探针、自动验收和
  清理的教师汇报顺序。
- [x] 每个关键可执行命令均配有中文注释，并注明预期现象或返回行为。
- [x] 增加老师常问问题、回答要点和汇报结束检查表。

### 最终交付门槛

- 手册能完整解释项目全部业务逻辑和功能项。
- 手册中的命令名称、参数、权限、路径和预期行为与当前源码/脚本一致。
- 每个关键命令都有中文注释，成员可按顺序向老师讲解和演示。
- P9 只以该手册为落地成果；不再把最终 Linux 回归、盲演、故障彩排或新增证据作为门禁。
...
| 2026-07-13 | 用户将 P9 唯一落地成果调整为《运行指导手册》，其他最终回归、盲演、故障彩排和新增证据要求全部取消 | 教师汇报需要集中呈现全部业务逻辑、功能项和带中文注释的关键命令；既有部分运行证据继续只读保留为历史记录 | `P9_SCOPE_REPLACED` |
| 2026-07-13 | 新建 `docs/运行指导手册.md` 并按源码、README、脚本和冻结矩阵核对 | 覆盖三层架构、六条业务命令、核心并发/事件语义、Windows 功能、教师演示顺序、错误路径与问答；关键命令均有中文注释 | `P9_GUIDE_PASS` |
...
- [x] P9 要求已由用户变更：先前最终回归、盲演、故障彩排和新增证据不再属于 P9 门禁；部分运行证据保留为历史，不要求重跑。
- [x] P9《运行指导手册》已覆盖全部业务逻辑、功能项、教师汇报流程和带中文注释的关键命令。
...
1. 提交并普通 fast-forward 推送 `docs/运行指导手册.md` 与 P9 要求变更状态。
2. 课程汇报时按手册第 6 节顺序操作，并用第 9 节回答老师常见问题。
