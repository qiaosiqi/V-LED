...
> 状态：`SPEC_FROZEN / P1_TARGET_LINUX_PASS / P2_WINDOWS_PASS / P3_TARGET_PASS / P4_TARGET_EVIDENCE_PASS / P5_TARGET_PASS / P6_TARGET_PASS / P7_CONTENT_PASS / P8_LAYOUT_PASS / P9_GUIDE_PASS`
>
...
6. P5 中只有版本实际变化才使独立 FD 的 poll/read 条件成立；成功但无变化的写入
   可以唤醒 wait queue，让 `fork/dup` 共享同一 file context 的 reader 处理本地刷新
   标志，但其他 FD 重新检查后仍继续等待。
...
|---|---|---|---|---|---|
| PDF4-01 | Linux 模块动态扩展，无需重启 | `vled_init/vled_exit` | `T-LIFE-01..04` | build、insmod、rmmod、dmesg | `PASS` |
| PDF4-02 | 实现 open/read/write/close 回调 | `vled_fops`，close 对应 release | `T-FOPS-01..05` | probe/strace/驱动日志 | `PASS` |
| PDF4-03 | 设备号管理 | `alloc_chrdev_region` | `T-LIFE-02` | `/proc/devices`、`stat` | `PASS` |
| PDF4-04 | 字符设备注册 | `cdev_init/cdev_add` | `T-LIFE-01` | 模块加载日志 | `PASS` |
| PDF4-05 | 自动创建设备文件 | `class_create/device_create` | `T-LIFE-02..03` | `/dev/vled` 出现/消失 | `PASS` |
| PDF4-06 | 用户写入，内核接收并处理 | `vled_write`/命令解析 | `T-CMD-*` | 功能日志、状态 JSON | `PASS` |
| PDF4-07 | 用户读取内核返回状态 | `vled_read`/JSON | `T-READ-*` | 功能与快照日志、JSON 解析结果 | `PASS` |
| PDF4-08 | 内置一页大小内核缓冲区 | PAGE_SIZE 共享状态缓冲区 | `T-BUF-*` | 边界日志、源码/构建 | `PASS` |
| PDF4-09 | 多进程同开、独立读写偏移 | per-open context | `T-FD-*`、`T-MP-*` | 双 FD 与真实 fork 多进程探针、并发日志 | `BASELINE_PASS / T-MP_TARGET_PENDING` |
| PDF4-10 | 动态装卸和节点注册测试 | 生命周期脚本 | `T-LIFE-*` | 完整生命周期日志 | `PASS` |
...
### 8.1 真实多进程、锁顺序与死锁对照验收

原 T-CON 使用 Python 线程并发，不能单独证明两个独立进程的 `open()`、地址空间和
文件上下文语义。新增 `tools/vled_multiprocess_probe.py` 使用 Linux `fork` 创建
真实子进程，并把“正常驱动测试”和“故意死锁示范”严格隔离。

| ID | Given/When | Then |
|---|---|---|
| T-MP-01 | reader 进程长期打开设备并消费初始版本，writer 进程独立 open 后写入实际变化 | reader 被 wait queue 唤醒并读到 writer 发布的新 version；限定时间内退出 |
| T-MP-02 | reader 进程读取旧 JSON 前缀，writer 进程更新状态并用新 FD 读取 | reader 拼接出完整旧快照，writer 读到完整新快照；两者偏移互不影响 |
| T-MP-03 | 4 个 writer 进程与 4 个 reader 进程同时执行规定次数 | 所有子进程在超时前退出，每次读取均为合法完整 JSON，version 不倒退 |
| T-MP-04 | `fork()` 后父子共享同一 open file description；reader 消费当前版本后阻塞，writer 在同一 FD 写 `STATUS` | reader 被刷新唤醒，读取状态与 version 均不变；旧实现应在该项超时 |
| T-DEADLOCK-01 | 两个一次性用户态子进程分别按 A→B 和 B→A 获取锁 | 父监视器观察双方已持有第一把锁且无进展，打印检测结果并终止子进程；不影响内核 |
| T-DEADLOCK-02 | 两个用户态子进程都按 A→B 获取锁 | 两者在限定时间内完成，证明固定全局锁序消除循环等待 |
...
`read()`、`write()`、`.poll()` 不得出现反向获取。故意在内核中构造死锁会让实验机
无法安全继续测试，因此 T-DEADLOCK-01 只在可终止的用户态子进程中展示 ABBA 反例；
真实驱动以 T-MP-01..04、T-CON-01..05 和内核日志共同证明无死锁和资源争夺错误。

2026-07-13 本地预检已完成：新增 Python 文件通过语法解析，P5 静态契约和多进程
静态契约通过，Windows 模拟器 17 项单元测试通过。由于本地不是 Linux 内核模块
环境，T-MP-01..04、修改后模块构建和统一验收当前标记为
`T-MP_TARGET_PENDING`；必须以目标 Ubuntu 上的实际输出关闭，不能复用历史 P5/P6
证据代替。
...
`568c263`。Windows 侧静态契约、Python/C/shell 语法和模拟器回归已通过；
T-POLL-01..08 最终已在目标 Linux 的明确提交 `e142f20` 上通过。
...
第二次目标运行中 T-POLL-01..07 与 T-POLL-08 均通过，但 P1 `T-FD-03` helper
在读完整个旧快照后又调用一次 `read()`，跨入已就绪的新版本并将两个 JSON 拼接
比较。修复批次按旧快照的已知剩余长度精确读取，只验证旧快照内部一致性；在
P1–P4 全回归通过前，P5 仍不标记为 PASS。

第三次目标运行在明确提交 `ceddae9` 上通过 P1 探针和 T-POLL-01..08；完整
`vled_verify.sh` 的 T-CLI/T-CMD 通过，但 T-CON 的 Python 分段 reader 在完整
...
| 20260712-2147-p6 | `2ea2346` | Ubuntu 24.04.4 / 6.17.0-35 / GCC 13.3 | 规范工具严格构建、bridge/P1/P3/P5 回归、三轮生命周期、内核日志 | `PASS / P6_TARGET_PASS` | `docs/evidence/20260712-2147-ubuntu-6.17.0-35-generic-p6/` |
| 20260713-1300-p9-partial | pull 到 `ba1ab82`；完整哈希未打印 | Ubuntu / 6.17.0-35-generic / GCC 13 | P1–P5、500 次并发、20 轮生命周期和验收器内核区间实际输出 | `FUNCTIONAL_OUTPUT_PASS / EVIDENCE_CAPTURE_FAIL` | `docs/evidence/20260713-1300-ubuntu-6.17.0-35-generic-p9-partial/` |

## 14. P9《运行指导手册》验收

用户于 2026-07-13 将 P9 唯一落地成果调整为 `docs/运行指导手册.md`。先前规划的
最终回归、盲演、故障彩排和新增证据不再属于 P9 门禁；该调整不改变第 2–11 节已经
冻结并验证的实现语义。

| ID | 验收项 | 通过条件 | 当前状态 |
|---|---|---|---|
| T-P9-GUIDE-01 | 业务逻辑完整性 | 覆盖三层架构、生命周期、PAGE_SIZE、per-open、快照、回滚、version、poll、bridge 和模拟器线程模型 | `PASS` |
| T-P9-GUIDE-02 | 功能项完整性 | 覆盖六条业务命令、JSON 字段、Windows 界面、边界和错误路径 | `PASS` |
| T-P9-GUIDE-03 | 关键命令中文注释 | Windows/Linux 构建、加载、读写、演示、探针、验收和清理命令均有中文注释 | `PASS` |
| T-P9-GUIDE-04 | 教师汇报可用性 | 有建议顺序、预期现象、问答要点和结束检查表 | `PASS` |
| T-P9-GUIDE-05 | 实现一致性 | 工具名、参数、范围、权限和行为与源码、README、PLAN、冻结矩阵一致 | `PASS` |

以上五项全部通过，P9 状态为 `P9_GUIDE_PASS`。
